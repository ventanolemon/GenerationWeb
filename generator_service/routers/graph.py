...
def get_resources() -> dict:
    """
    Поставочные файлы для файловых параметров (словари, картинки, пулы).

    Только чтение и только то, что лежит в `resources/` рядом с
    приложением, — это содержимое продукта, одинаковое для всех, и
    личности оно не требует ровно по той же границе, по которой её не
    требует каталог узлов (§8: встроенное принадлежит продукту).
    Загрузка своих файлов — отдельная работа, см. open_items §3.1.
    """
    return graph_api.build_resources()
