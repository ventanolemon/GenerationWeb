...
export const api = {
  ...
  /**
   * Состояние развёртывания. Только суперадминистратор: обычному
   * администратору организации 403, и это нормальный ответ, а не сбой.
   */
  adminDeployment(id: Identity): Promise<DeploymentState> {
    return request<DeploymentState>("/api/admin/deployment", {
      headers: idHeaders(id),
    });
  },
