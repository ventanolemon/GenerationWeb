...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      // База знаний — туда же и по той же причине: правка страниц это
      // работа со СБОРКОЙ приложения, а не с учебными данными, которыми
      // распоряжается web_layer. Ровно как graph-роутер, и с тем же
      // следствием для развёртывания: перед FastAPI нужен тот же
      // маршрут, что и для /api/graph.
      "/api/guide": {
        target: "http://localhost:8000",
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/api/, ""),
