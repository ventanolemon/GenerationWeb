...
### Редактор графов: /api/graph/*

Тонкий релей в `generator_service` — там импортирован движок графов,
поэтому логика живёт там, а здесь только проброс.

```
GET  /api/graph/catalog     каталог узлов для палитры
GET  /api/graph/resources   поставочные файлы для файловых параметров
POST /api/graph/validate    {graph} → структура и типы портов
POST /api/graph/preview     {graph, seeds?} → блоки условия и ответа
```

Появилось не сразу, и отсутствие было ДЕФЕКТОМ РАЗВЁРТЫВАНИЯ: раньше
`/api/graph` отправлял на FastAPI dev-сервер Vite, а в бою Vite нет —
запрос приходил сюда и получал 404. Редактор графов разворачивался
только вместе с dev-сервером.

### База знаний: /api/guide

```
GET    /api/guide            страницы с наложенными правками (всем, включая гостя)
PUT    /api/guide/{page}     сохранить правку (админ, разработчик)
DELETE /api/guide/{page}     вернуть поставочную (админ, разработчик)
```

Права проверяет `generator_service`; здесь, как и везде в этом слое,
только релей. Чтение открыто гостю намеренно: база знаний открыта до
входа, и закрытое чтение правок означало бы, что гость и вошедший видят
разные страницы.
