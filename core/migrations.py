...
# ---------- Миграция 011: набор ключей подписи и его ротация ----------

def _m011_signing_keys(conn: sqlite3.Connection) -> None:
    """
    Ротация ключа выпуска.

    До неё ключ был один и жил в переменной окружения. Это работает ровно до
    первой неприятности: потерял ключ — обновляй всех клиентов вручную, они
    носят зашитым старый публичный и другого не примут.

    Решение — не «ключ», а НАБОР ключей, и набор этот подписан. Клиент несёт
    зашитым не ключ, а первый набор; получив новый, он принимает его, только
    если тот подписан ключом, активным в наборе, которому клиент уже верит.
    Так доверие переносится по цепочке, и смена ключа перестаёт означать
    переустановку.

    Храним ПОДПИСАННЫЙ АРТЕФАКТ ЦЕЛИКОМ (`payload` + `signature`), а не
    разобранные поля: клиент обязан проверять подпись ровно тех байтов, что
    подписывали. Собери мы payload заново из колонок — любое расхождение в
    сериализации сломало бы проверку у всех сразу.

    `sequence` монотонен: без него отозванный набор можно было бы подсунуть
    обратно, вернув к жизни скомпрометированный ключ. Подпись от такого
    отката не защищает — она валидна.

    Чего эта схема НЕ решает, сказано прямо в app_updates.md: ротацию при
    КОМПРОМЕТАЦИИ. Укравший приватный ключ подпишет ротацию на свой — тут
    помогает только доставка нового набора вне канала (новая сборка,
    переустановка).
    """
    conn.executescript("""
