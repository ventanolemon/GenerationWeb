...
def create_organization(repo: Repository, *, name: str,
    ...
    Родителя задать НЕЛЬЗЯ, и отказ здесь громкий — см. ниже.

    Иерархия университет → институт → кафедра заложена в схеме списком
    смежности (`organizations.parent_id`, миграция 014 §8.3), а
    НАСЛЕДОВАНИЯ нет: ни выдача с уровня университета не каскадирует на
    кафедры, ни руководитель института не видит содержимое своих кафедр.
    Это продуктовые решения, и принимать их вслепую заранее хуже, чем не
    принимать.

    Пока их нет, значение принимать нельзя. Записанный `parent_id`, на
    который никто не смотрит, — это ровно та «форма без семантики», про
    которую в `models_on_july.md` сказано, что она хуже отсутствия:
    администратор выстраивает структуру университета, видит её в ответе
    и вправе ждать, что выдача пойдёт по ней. Она не пойдёт, и узнает он
    об этом не здесь, а когда кафедра не увидит курс.

    Отказ, а не молчаливое игнорирование: проглоченное поле — та же
    ложь, только без следа. Колонка при этом остаётся: заложить форму
    дёшево, а снять отказ, когда семантика появится, — одна строка.
    """
    clean = (name or "").strip()
    if not clean:
    ...
    if parent_id is not None:
        raise OrganizationActionError(
            "Вложенные организации пока не поддерживаются: наследование "
            "прав и содержимого не реализовано, и родитель ни на что не "
            "повлиял бы. Заведите организацию без родителя.")
    if owner_login is not None:
        _require_user(repo, owner_login)
    # parent_id здесь всегда None (выше отказ), и передаётся он явно, чтобы
    # слой доступа не пришлось трогать, когда семантика появится.
    org_id = repo.create_organization(clean, parent_id=None,
