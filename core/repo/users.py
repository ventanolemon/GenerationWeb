...
class UsersMixin:
    ...
    def reset_user_password(self, login: str, new_password: str) -> bool:
        """
        Задать пароль БЕЗ проверки старого — восстановление доступа.

        Отдельно от `change_user_password` намеренно: там проверка старого
        пароля и есть весь смысл, здесь она невозможна по условию (пароль
        как раз и потерян). Разные операции с разными правами: смену
        делает сам пользователь, сброс — администратор развёртывания или
        скрипт заведения служебных аккаунтов.

        Хэширование живёт здесь, а не у вызывающего: второй способ
        положить пароль в базу однажды разойдётся с первым, и разойдётся
        молча — вход просто перестанет принимать верный пароль.
        """
        with self._connect() as conn:
            cur = conn.execute(
