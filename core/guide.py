...
#: То же, но БЕЗ требований к адресу — чтобы поймать ссылку, которая
#: выглядит как `guide:`, а адрес имеет недопустимый.
#:
#: Замер, из-за которого проверка появилась: `[текст](guide:нет-такой)` с
#: кириллицей проходил насквозь. Под запрет «ссылки не через guide:» он не
#: попадал (приставка на месте), в список ссылок не извлекался (адрес не
#: подошёл под шаблон) — и на странице оставался СЫРЫМ текстом со
#: скобками. Ровно тот случай, ради которого набор разметки и объявлен
#: закрытым: неизвестное — ошибка разбора, а не «покажем как есть».
_LINK_LOOSE = re.compile(r"\[[^\]]*\]\(guide:([^)]*)\)")
_ID = re.compile(r"\A[a-z0-9][a-z0-9-]*\Z")

#: Разметка, которую умеет показывать веб-клиент. Список закрыт НАРОЧНО.
#:
#: Без него формат «расширяется» молча: автор пишет таблицу, рендерер про
#: таблицы не знает, и в документации появляется абзац из палок. Поймано
#: ровно так — нумерованный список схлопнулся в один абзац, и увидеть это
#: можно было только глазами на собранной странице.
#:
#: Поэтому неизвестная разметка — ОШИБКА РАЗБОРА. Расширять формат можно,
#: но только вместе с рендерером и этим списком.
_UNSUPPORTED = (
    (re.compile(r"^\s*\|"), "таблицы"),
    (re.compile(r"^\s*>"), "цитаты"),
    (re.compile(r"^#{4,}\s"), "заголовки глубже третьего уровня"),
    (re.compile(r"^#\s"), "заголовок первого уровня (он берётся из title)"),
    (re.compile(r"!\[[^\]]*\]\((?!shot:)"), "картинки по пути — ставьте shot:"),
    (re.compile(r"(?<!!)\[[^\]]*\]\((?!guide:)"), "ссылки не через guide:"),
)
...
def parse_page(path: pathlib.Path) -> Page:
    """Разобрать файл источника."""
    return parse_text(path.read_text(encoding="utf-8"), path.name)


def parse_text(raw: str, source: str = "страница") -> Page:
    """
    Разобрать страницу ИЗ ТЕКСТА.

    Отделено от чтения файла ради правки страниц из приложения
    (`core/guide_store.py`): текст, присланный редактором, обязан
    проходить ТУ ЖЕ проверку формата, что и файл поставки. Иначе
    администратор сохранит таблицу или заголовок без адреса, разбор
    промолчит, и страница сломается у всех читателей сразу.

    `source` попадает в сообщения об ошибках — для файла это имя файла,
    для правки из приложения имя страницы.
    """
    ...
    for number, line in enumerate(body.splitlines(), start=1):
        if line.startswith("```"):
            ...
            continue
        if fenced:
            continue                    # внутри врезки кода что угодно
        for pattern, what in _UNSUPPORTED:
            if pattern.search(line):
                raise GuideError(
                    f"{source}, строка {number}: {what} веб-клиент не "
                    f"показывает. Разметка базы знаний — закрытый набор: "
                    f"расширять её можно только вместе с рендерером.")

    for match in _LINK_LOOSE.finditer(body):
        address = match.group(1)
        if not _LINK.match(match.group(0)):
            raise GuideError(
                f"{source}: адрес ссылки {address!r} недопустим. Адрес — "
                f"это `страница` или `страница/раздел` из строчных "
                f"латинских букв, цифр и дефисов. Ссылка с другим адресом "
                f"не распознаётся клиентом и остаётся на странице сырым "
                f"текстом.")
