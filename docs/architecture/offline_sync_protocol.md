...
>    Область видимости считается реальным RBAC Фазы 1
>    (`Repository.visible_subject_ids`). Устройство без identity получает
>    ТОЛЬКО встроенные предметы: «видно всё» было dev-заглушкой с тех
>    времён, когда web_layer не пробрасывал личность, и после auth-фазы
>    оказалось дырой — свежая установка до входа вытягивала чужой
>    авторский контент. Пустой скоуп тоже не годится: без общего каталога
>    клиент не работает офлайн до входа.
...
### Scope-эпоха: изменение ПРАВ как событие синхронизации

Курсорный диф переносит изменения содержимого, но не изменения прав, и
молчит он в обе стороны: выдали предмет — его `row_version` старая, курсор
клиента её давно прошёл, предмет не приедет никогда; отозвали — версия не
менялась вовсе, события нет. Поэтому у каждого пользователя есть счётчик
`users.scope_version`, растущий при любом изменении его видимого набора
(выдача, отзыв, смена режима умолчания, смена роли — миграция 006).

Запрос pull дополнен полем `scope_version` (известная клиенту; `0` — не
знает), ответ — полями `scope_version` (актуальная серверная) и `resync`
(эпоха клиента устарела). При `resync: true` сервер игнорирует курсоры и
отдаёт полный доступный набор; клиент применяет его, затем удаляет у себя
всё, чего в наборе не было, и только после этого запоминает эпоху — обрыв
до этой точки безопасен, следующий sync начнёт пересборку заново.

Два решения этой реализации, которых модель прав сама по себе не диктует:

1. **Пересборка приходит одним ответом, без пагинации.** Клиент в режиме
   пересборки шлёт пустые курсоры на каждой странице (иначе он потерял бы
   «с нуля»), а сервер по отношению к клиентам stateless и вторую страницу
   от первой не отличает — пагинированная пересборка не сошлась бы. Обрезать
   набор вместо этого нельзя: клиент удаляет всё, что не приехало, и
   обрезанная страница означала бы удаление законно выданного контента.
2. **Собственные предметы преподавателя не выпадают из скоупа никогда**,
   каким бы ни был набор выдач: чистка на клиенте удаляет невыданное, и
   выпади оттуда авторский контент — отзыв доступа к ЧУЖОМУ предмету стёр бы
   преподавателю его собственную работу. Витрину его же предметов при этом
   всё равно ограничивает клиентский фильтр, а он ничего не удаляет.

Стартовое значение эпохи — 1, а не 0: клиент, который эпохи ещё не знает,
шлёт 0 и так гарантированно получает пересборку на первом же pull, вместо
того чтобы совпасть с сервером по случайности нуля.

Модель выдач целиком — `Generator/docs/subject_grants.md`.
