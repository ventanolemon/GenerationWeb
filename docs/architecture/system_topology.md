...
1. Клиенты не ходят мимо `web_layer` (в т.ч. десктоп: всё онлайн-общение
   ...
   *Инвариант нарушался, и нарушался незаметно.* Vite отправлял
   `/api/graph` (а позже и `/api/guide`) прямо в `generator_service`, и в
   разработке всё работало. Обнаружилось это не как нарушение
   инварианта, а как дефект развёртывания: в бою Vite нет, запрос
   приходит в `web_layer`, а маршрута там не было — редактор графов
   разворачивался только вместе с dev-сервером.

   Урок общий: **инвариант, который проверяется только чтением, не
   проверяется.** Обход жил в конфигурации сборщика, то есть там, где
   этот документ его не искал. Релеи добавлены, особые правила Vite
   убраны — теперь путь разработки совпадает с боевым, и следующее такое
   расхождение упрётся в неработающую разработку, а не в продакшен.
