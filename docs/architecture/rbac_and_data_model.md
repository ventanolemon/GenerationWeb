...
>
> **Чего пока нет — авторизации ЧТЕНИЯ разделов.** `GET /partitions/{id}`
> отдаёт `generation_params`, то есть весь авторский граф, по id и без
> проверок; `GET /partitions/candidates/{subject_id}` — так же. Закрывать
> это надо тем же скоупом выдач, что и pull синка
> (`sync_api.visible_scope`), и это отдельная задача: смешивать её с
> закрытием дыры на запись значило бы менять область видимости всей витрины
> заодно. До тех пор `generator_service` обязан оставаться закрытым от
> внешней сети (он и задуман внутренним: см. `system_topology.md`).
