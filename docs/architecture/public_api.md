...
1. **Не открывать `generator_service` наружу.** Он спроектирован
   внутренним. Запись разделов теперь авторизована
   (`core/content_authz.py`), но ЧТЕНИЕ — нет: `GET /partitions/{id}`
   отдаёт весь авторский граф по id кому угодно, кто дотянулся до сервиса
   (см. `rbac_and_data_model.md` §3). Публичный вход — только через
   `web_layer`.
