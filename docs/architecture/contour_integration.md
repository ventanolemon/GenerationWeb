...
> **Статус реализации: contour_service реализован** (`contour_service/`:
> очередь, реестр провайдеров, петля S0–S5, corpus-персист с graph_hash-
> дедупом, API §2/§4; 33 headless-теста). Два отклонения от текста ниже,
> продиктованные окружением. (1) Рядом с боевой Postgres-очередью
> (FOR UPDATE SKIP LOCKED, `queue.PostgresJobQueue`) добавлена
> `SqliteJobQueue` за тем же интерфейсом `JobQueue`: dev/тестовое окружение
> монорепо живёт на SQLite без PG-сервера, а интерфейс-шов и так был
> предусмотрен system_topology §4. (2) Интеграционные тесты помечаются не
> `@pytest.mark.integration`, а `unittest.skipUnless(env)` — pytest в
> окружении монорепо отсутствует, семантика та же.
