...
| Развернуть на машине | `deploy/` — четыре контейнера, `docker compose up` |
...
Работает автономно: своя БД, своё ядро, никакой сети. Гостевой вход —
кнопка на экране авторизации.

**База и поставка — разные файлы.** `resources/users_database.db` — это
ШАБЛОН, он только читается. Рабочая база при первом запуске создаётся его
копией в `GEN_DATA_DIR` (по умолчанию `data/` рядом с проектом), и дальше
приложение пишет только туда. Перенести установку на другую машину —
скопировать `data/`; начать с нуля — удалить его.
...
## Развёртывание: четыре контейнера
...
Отличие от таблицы выше — не в количестве процессов, а в том, кто виден
снаружи. Наружу смотрит **только nginx**: он раздаёт статику клиента и
уводит `/api` в `web_layer`. У `generator_service` и `contour_service`
проброшенных портов нет вовсе, и заголовки личности, пришедшие с улицы,
nginx срезает — иначе браузер ставил бы их сам и роль перестала бы
что-либо значить.

Рабочая база живёт на томе, а `resources/users_database.db` в образе —
шаблон, с которого том наполняется при первом запуске. Именно ради этого
поставка и данные разведены: слой образа доступен только на чтение.

Подробности, и в том числе честный список того, для чего этот пример НЕ
годится (SQLite вместо Postgres под очередью, отсутствие TLS и резервных
копий, одна реплика), — в [`deploy/README.md`](../../deploy/README.md).
