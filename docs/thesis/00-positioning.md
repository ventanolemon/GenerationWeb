...
| Р3 | Корректность генератора заданий проверяется прогоном, а не доказательством: генератор порождает пары «условие — ответ», и согласованность пары можно проверять массово | набор из 3797 проверок, включая сверку вычисленного ответа с независимо посчитанным |
...
| формализация в принятой нотации | Capella / ARCADIA (см. [02. Архитектура](02-chapter-architecture.md)) |
...
|---|---|---|
| 1. Анализ и постановка | почему проверка ответа — нетривиальная задача; обзор; противоречие | Р1, замер столкновений |
| 2. Системный анализ и архитектура | ARCADIA OA/SA/LA/PA, границы, потоки | реальная топология |
| 3. Реализация | язык описания заданий, механизм проверки, автономность | 145 узлов, синхронизация |
| 4. Результаты и оценка | замеры, границы применимости, что не проверено | все прогоны |
...
flowchart TB
    ...
    S1["Статья С1<br/>допуск на опечатку<br/>= фрагмент главы 4"]
    S2["Статья С2<br/>ответ как данные<br/>= фрагмент главы 3"]
    S3["Статья С3<br/>верификация генераторов<br/>= фрагмент главы 4"]
    D --> S1
    D --> S2
    D --> S3
```
...
разделом главы 4 диплома. Дальше С3, для которой надо сформулировать
...
описания заданий и проверяется набором из 3797 автоматических проверок;
