...
|---|---|---|---|
| запись голоса | **реализовано** средствами системы | **не реализовано** | — |
| эталоны | из поставки локально | — | из поставки локально |
| признаки и расстояния | **локально** | — | **локально** |
| вердикт | локально | — | локально |
| хранение записи | **не хранится** | — | не хранится |

Столбцы «реализовано / не реализовано» проставлены по факту, а не по
замыслу. Браузерная половина отсутствует по названной причине: запись
приходит оттуда в сжатом формате, а проверка читает несжатый PCM, и
передавать её надо самими данными, а не путём к файлу — то есть через
переносимое содержимое, которого пока нет (глава 5, §5.11).
...
Цена решения замерена: 13 МиБ эталонов в поставке и **0,09 с в среднем,
0,21 с в худшем случае** на один ответ при окрестности из 12 слов. Первая
версия стоила 3,65 с — цифра приведена потому, что показывает, где здесь
настоящая цена: не в признаках, а в выравнивании, чья стоимость растёт
как произведение длин. Оценка «порядка десятка миллисекунд на слово
словаря» подтвердилась ПОСЛЕ перестройки вычисления, а не до неё, и
записана в главе 7 вместе с тем, как получена.

**Хранение записи в размещении отсутствует намеренно.** Вердикт
вычисляется на том же узле, где сделана запись, и наружу идёт результат,
а не голос. Это снимает с физической архитектуры весь вопрос о хранении
персональных данных особой категории — до тех пор, пока хранение не
понадобится; когда понадобится, решать его придётся явно, а не
обнаружить уже накопленным.
