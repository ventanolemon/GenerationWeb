...
| **произношение** | **запись, чей ближайший эталон — целевое слово; уверенность меряется расстоянием МЕЖДУ эталонами словаря** | **было 144 уверенно неверных на 11 словарях из 14, стало 0 на всех (§7.3.2)** |
...
```mermaid
flowchart TB
    R["Правило окрестности:<br/>мера ошибки задаётся расстоянием<br/>до ближайшего ДРУГОГО допустимого ответа"]
    T["Текст:<br/>расстояние Левенштейна<br/>между терминами словаря"]
    A["Звук:<br/>расстояние выравнивания<br/>между эталонами словаря"]
    RT["Сколько правок прощать:<br/>допуск = min(политика длины,<br/>d_ближайший − 1)"]
    RA["Насколько верить выбору:<br/>доля = отрыв / separation,<br/>вердикт при доле ≥ 0.30"]
    RA2["Что выбрать:<br/>эталон целевого слова<br/>ближе прочих"]
    R --> T --> RT
    R --> A --> RA
    A --> RA2
```

*Рис. 5а.2. Одно правило, две модальности — и в звуке оно отвечает на два
вопроса, а не на один.*

Разница в форме, но не в существе: и там, и там **мера ошибки задаётся
остальными допустимыми ответами, а не константой**.

**Про звук это верно в двух местах, и второе появилось не сразу.** Выбор
ближайшего эталона окрестностью задан с самого начала; а вот
«достаточно ли уверен выбор» сначала решалось константой — то есть ровно
тем, против чего правило и написано. Замер по всем словарям поставки это
опроверг (144 уверенно неверных вердикта на 11 словарях из 14), и мера
уверенности была выведена из той же окрестности: отрыв меряется долей от
расстояния МЕЖДУ эталонами словаря. После замены — 0 уверенно неверных
на всех 14 словарях (§7.3.2).

Наблюдение, которое стоит вынести из этого случая в архитектурный вывод:
**применить принцип наполовину — значит не применить его.** Половина,
оставшаяся на константе, определила поведение всего механизма, и никакая
правильность первой половины этого не компенсировала.
