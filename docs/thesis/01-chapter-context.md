...
| Pr.02 | Проверка отвергает верные ответы, записанные в другой форме: `0,5` / `1/2` / `2/4`, `x^2` / `x*x`. Каждый такой случай — спор, который разбирает преподаватель вручную | St.01, St.02 | **требует замера** |
...
### Замер Pr.02 — частота отказов по форме

**Что измеряется:** доля ответов, отвергнутых при строгом сравнении, но
признанных верными при разборе.

**Как:** прогнать накопленные попытки (таблица `attempts` уже содержит
введённый студентом текст) двумя проверками — строгой и предметной — и
сравнить вердикты. Ручной разбор нужен только для расхождений.

**Что даст:** Pr.02 в числах на реальных ответах, без организации
эксперимента.

**Замечание о честности работы.** Второй замер выполним прямо сейчас на
имеющихся данных, первый требует организации. В главе 7 отсутствие обоих
названо прямо, и выводы работы сформулированы так, чтобы от них не
зависеть.
...
1. Процесс выдачи и проверки индивидуальных заданий содержит три
   системных дефекта (Pr.03, Pr.04, Pr.06), которые не устраняются
   организационными мерами: они встроены в способ построения проверки.
...
4. Два показателя проблем не замерены; для обоих приведена выполнимая
   методика сбора, а выводы работы построены так, чтобы от них не
   зависеть.
