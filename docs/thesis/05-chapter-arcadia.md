...
## 5.7. Уровень LA — логическая архитектура (обзор)

Семь логических компонентов первого уровня; **подробный разбор — в
[главе 5а](05a-logical-architecture.md)**: 26 подкомпонентов, интерфейсы,
распределение системных функций, роли и сценарии на уровне LA.

```mermaid
flowchart TB
    LC1["ЛК1 · Описание задания"]
    LC2["ЛК2 · Порождение варианта"]
    LC3["ЛК3 · Критерий ответа"]
    LC4["ЛК4 · Прохождение и учёт"]
    LC5["ЛК5 · Согласование установок"]
    LC6["ЛК6 · Доступ"]
    LC7["ЛК7 · Подтверждение корректности"]
...
*Рис. 5.3. Логические компоненты первого уровня.*

Разбиение проведено **по владению понятием**, а не по слоям: компонент
отвечает за одно понятие предметной области целиком. Проверка качества
разбиения — каждая из восьми системных функций имеет ровно одного
владельца (таблица 5а.1).

Центральный компонент — **ЛК3 (критерий ответа)**: в нём сосредоточен
результат работы, и в нём единственный подкомпонент, обслуживающий две
модальности сразу — правило окрестности, применённое и к тексту, и к
звуку (§5а.4).
...
## 5.10. Уровень PA — физическая архитектура (обзор)

Шесть узлов развёртывания; **подробный разбор — в
[главе 5б](05b-physical-architecture.md)**: размещение компонентов по
узлам, семь физических интерфейсов, 29 таблиц хранилища по владельцам,
три способа развёртывания и меры контроля расхождения.

```mermaid
flowchart TB
    subgraph N1 [У1 · Рабочее место]
        D1["Оболочка"]
        D2["Копия движка"]
        D3[("Локальная база")]
    end
    subgraph N2 [У2 · Сервер приложения]
        S1["Доступ, маршрутизация, обмен"]
    end
    subgraph N3 [У3 · Служба порождения]
        S2["Копия движка, сессии"]
    end
    W1["У5 · Браузер"]
    N4["У4 · Служба нечётких задач"]
    F[("У6 · Хранилище")]

    W1 --> S1
    D1 -->|"обмен при связи"| S1
    S1 --> S2
    S1 --> N4
    S2 --> F
    S1 --> F
...
*Рис. 5.4. Узлы развёртывания.*

**Главное свойство состава: обязательных узлов нет ни одного.** Система
разворачивается тремя способами — только рабочее место (автономно),
сервер с браузером (только веб), полный состав. Это прямое следствие
требования СТ4 и то, что отличает её от типовой клиент-серверной схемы.

**Главное следствие того же требования:** пятнадцать подкомпонентов
существуют в ДВУХ копиях. Отсюда риск, которого на уровне LA не было, —
расхождение копий, причём два вида расхождения из четырёх проявляются
молча. Меры контроля и их замеры — в §5б.3.
...
| **LA** | LAB (Logical Architecture Blank) | 7 компонентов первого уровня, 26 подкомпонентов (гл. 5а) |
| | LDFB (Logical Data Flow) | распределение 8 системных функций, таблица 5а.1 |
| | 7× LFBD (Logical Function Breakdown) | декомпозиция каждого ЛК |
| | 3× ES (Exchange Scenario) | ЛС-1 прохождение, ЛС-2 произношение, ЛС-3 обмен |
| | Interfaces | 20 логических интерфейсов И1.1–И6.3 |
| **PA** | PAB (Physical Architecture Blank) | 6 узлов, размещение по таблице 5б.2 |
| | PDFB (Physical Data Flow) | 7 физических интерфейсов ФИ1–ФИ7 |
| | PCBD (Physical Component Breakdown) | состав У1 и У3 (дублированные компоненты) |
| | Deployment | три способа развёртывания, таблица 5б.8 |

**Таблицы прослеживания в Capella — важнее диаграмм:**

* Operational Capability → System Function (OC → F);
* System Requirement → Logical Component (СТ → ЛК), таблица 5а.3;
* Logical Component → Physical Component (ЛК → узел), таблица 5б.2;
* System Function → Logical Component (F → ЛК), таблица 5а.1;
* System Function → Mode/State (F → S), глава 4;
* Actor → Logical Component (роль → ЛК), таблица 5а.2.
...
2. На каждом переходе между уровнями проведён анализ альтернатив
   ...
   Подробные уровни LA и PA вынесены в главы
   [5а](05a-logical-architecture.md) и
   [5б](05b-physical-architecture.md): 26 подкомпонентов с интерфейсами
   и 6 узлов с размещением.
