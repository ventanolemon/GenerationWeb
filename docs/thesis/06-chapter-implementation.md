...
**Различие модальности проявилось и в том, ЧЕМ ответ передаётся.**
Настольное приложение отвечает путём к записи: она снята и проверена в
одном процессе на одной машине. По сети путь бессмыслен — он назвал бы
файл на машине студента, — поэтому у веб-клиента форма третья: сама
запись, `data:audio/wav;base64,…`. Это не обход, а следствие того же
разделения: ядро говорит о ЗВУКЕ, а не о файле, и способ доставки звука
— свойство канала, а не правила приёма. Хранилища при этом не
понадобилось: вердикт считается на месте, в попытку идёт результат.

Приведение частоты выполняет клиент — тем же линейным алгоритмом, что и
ядро. Совпадение здесь обязательно: правило сравнивает РАССТОЯНИЯ, и
разные способы передискретизации дали бы двум клиентам разные признаки
из одной записи, то есть разные вердикты об одном произношении.
