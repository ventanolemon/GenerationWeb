...
**Что из этого следует делать** — вывести порог уверенности из той же
окрестности. Это сделано, и §7.3.2 отчитывается о результате.

---

## 7.3.2. Исправление: порог уверенности выводится из окрестности

### Величина, которая приносится словарём

Ошибка §7.3.1 состояла не в числе 0,08, а в том, что уверенность вообще
меряли **константой**. Правило окрестности было применено к первому
вопросу («какое слово ближе») и не применено ко второму («достаточно ли
уверенно») — при том что второй вопрос ровно того же рода.

Нужная величина есть в самом словаре:

> **separation(W)** — расстояние от эталона слова W до ближайшего
> **чужого** эталона того же словаря.

Она считается **по одним эталонам** и о записи студента не знает ничего,
поэтому не зависит ни от голоса, ни от микрофона, ни от уровня шума —
то есть калибровки не требует по построению, а не по удачному стечению
обстоятельств.

Дальше — простое рассуждение о достижимом. Идеальная запись слова W
совпала бы с его эталоном: расстояние до своего 0, до ближайшего чужого
ровно separation(W). Значит **отрыв, равный separation(W), — это
максимум, который на данном словаре вообще можно взять.** Отсюда
безразмерная величина

```
share = (расстояние до второго − расстояние до первого) / separation(W)
```

— «какую часть достижимого запись действительно взяла», примерно в
пределах [0, 1]. Вердикт выносится при `share ≥ 0,30`.

**Почему это самонастройка, а не переименованная константа.** На словаре
из похожих слов separation мал — достижимое разделение мало, и то же
требование «взять треть» автоматически означает меньший абсолютный
отрыв. На словаре из длинных различных терминов separation велик, и
требование становится строже в абсолютных единицах. Материал сам задаёт
масштаб; 0,30 — это **требование к качеству записи**, а не порог
расстояния.

### Результат

Оба правила прогнаны на ОДНОМ материале в одном прогоне (иначе сравнение
свелось бы к сопоставлению двух разных случайностей): 14 словарей,
окрестность 12 слов, 6 искажений, 25 проб на словарь — 2100 проверок.

**Таблица 7.5 — Прежнее и нынешнее правило уверенности**

| | Вердикт вынесен | Из них неверных | Словарей с ошибками |
|---|---|---|---|
| константа `отрыв/расстояние ≥ 0,08` | 1651 из 2100 (78,6%) | **144 (8,7%)** | **11 из 14** |
| окрестность `share ≥ 0,30` | 832 из 2100 (39,6%) | **0 (0,0%)** | **0 из 14** |

![Порог уверенности: было и стало](figures/pronunciation-spread.svg)

*Рис. 7.3. По два ряда на словарь: верхний — прежнее правило, нижний —
нынешнее. Рисунок ПОРОЖДАЕТСЯ замером
(`python -m scripts.figure_pronunciation`), а не собирается по его
пересказу: рис. 7.2 был собран руками и повторил ошибку счётчика вместе
с подписью «красного нет».*

**Проверка на невиденном материале.** Долю 0,30 подбирали на семи
словарях, отобранных по чётности контрольной суммы имени — то есть без
участия человека в выборе. На остальных семи, в подборе не
участвовавших: **0 уверенно неверных вердиктов**. Прежняя константа
этой проверки не проходила: настроенная до нуля ошибок на первой
половине, на второй давала 1,6%. Это и есть разница между величиной,
выведенной из данных задачи, и величиной, подогнанной под материал.

**Разброс между словарями исчез.** Доля вынесенных вердиктов лежит
в узком коридоре 34–45% на всех четырнадцати словарях, тогда как прежде
в коридоре 0–62% лежала доля ОШИБОК. Иначе говоря, механизм перестал
вести себя по-разному на разном материале — а это и было главной
претензией §7.3.1.

### Цена, названная прямо

**Покрытие упало вдвое: с 78,6% до 39,6%.** Половина ответов теперь
получает «не разобрал, повторите» вместо вердикта.

Это осознанный размен, и обоснование у него не эстетическое. Из двух
ошибок в учебной системе дороже та, которую студент не может оспорить:
«неверно» на верно произнесённом слове подрывает доверие к проверке
целиком, а «повторите отчётливее» стоит одной попытки. Замер даёт
основание для размена в числах: 144 несправедливых «неверно» против 819
дополнительных просьб повторить.

Распределение цены по искажениям показывает, где именно она платится:

| Искажение | Вердикт выносится |
|---|---|
| громкость ×0,5 | 100% (громкость на признаки не влияет по построению) |
| темп ×0,85 / ×1,20 | 75–82% |
| шум 5% | ~5% |
| шум 15%, темп+шум | ~2% |

**Шум съедает уверенность почти целиком, и это честно.** Отрыв под шумом
сжимается, а separation считается по чистым эталонам и не сжимается —
доля падает, и правило отказывается судить. Прежняя константа этого не
замечала именно потому, что делила отрыв на расстояние до ближайшего,
которое под шумом росло вместе с ним: отношение оставалось прежним, и
правило считало себя уверенным, будучи неправым.

**Чего это не отменяет.** Замер по-прежнему на синтетических искажениях
и синтезированных эталонах. Утверждать «система не ошибается уверенно»
теперь можно **на этом материале и при названном протоколе** — но не
как свойство работы с живыми записями: там не измерено ничего.
...
Всего автоматических проверок в комплексе — **3985** (сервер 1640,
настольное приложение 2274, веб-клиент 71).
